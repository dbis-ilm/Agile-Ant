...
package object config {
  ...
  private[spark] val AGILEANT_VOTING_THRESHOLD = // Agile-Ant: the default is #executors/3 (which we select experimentally as stated in Section 7.1.1 of the paper). Therefore, we make it as a configuration parameter to be tuned by the users upon additional experiments
    ConfigBuilder("spark.agileant.votingthreshold")
      .internal()
      .version("3.1.2")
      .doubleConf
      .createWithDefault(0.33)
    
  private[spark] val AGILEANT_DATASET_BENEFIT_RATIO = // Agile-Ant: the default is 0.5 (which we select experimentally as stated in Section 6.1.1 of the paper). Therefore, we make it as a configuration parameter to be tuned by the users upon additional experiments
    ConfigBuilder("spark.agileant.datasetbenefitratio")
      .internal()
      .version("3.1.2")
      .doubleConf
