...
private[spark] class BlockManager(
  ...
  val executorCacheManager = new ExecutorCacheManager()
  ...
  def initialize(appId: String): Unit = {
    ...
    if(!blockManagerId.isDriver){
      executorCacheManager.init(this) // each block manager (i.e. executor) is associated with an Executor-cache Manager  (Sect. 5 of the paper)
      memoryStore.SetExecutorCacheManager(executorCacheManager)
    }
    // Agile-Ant: End
  ...
  private def doPutIterator[T](
      ...
      keepReadLock: Boolean = false): Option[PartiallyUnrolledIterator[T]] = {
        val startTimeMs = System.currentTimeMillis // Agile-Ant: The timestamp when the block computation starts
    doPut(blockId, level, classTag, tellMaster = tellMaster, keepReadLock = keepReadLock) { info =>
        ...
        if (level.deserialized) {
          memoryStore.putIteratorAsValues(blockId, iterator(), classTag) match {
            ...
            case Left(iter) =>
              ...
              // Agile-Ant: Start
              if(blockId.isRDD) { 
                // profile that caching the block failed
                executorCacheManager.ProfileFailedToBeCached(blockId)
                executorCacheManager.ProfileBlockSize(blockId, memoryStore.s_estimated_size_B)
              }
              // Agile-Ant: End
          ...
          memoryStore.putIteratorAsBytes(blockId, iterator(), classTag, level.memoryMode) match {
            ...
            case Left(partiallySerializedValues) =>
              ...
              if(blockId.isRDD) { 
                // profile that caching the block failed
                executorCacheManager.ProfileFailedToBeCached(blockId) 
                executorCacheManager.ProfileBlockSize(blockId, 0)
              }
              // Agile-Ant: End
      ...
      // Agile-Ant: Start
      val requiredMemory = memoryStore.GetRequiredMemory() 
      if(!blockManagerId.isDriver && requiredMemory > 0){ 
        executorCacheManager.SendAMessageToTheClusterCacheManager(MessageType.SendMemoryStatusAndBlocksComputationTime_Msg, executorId.toInt, (requiredMemory, memoryStore.GetPredictedRddFullTargetSizeMap(), executorCacheManager.GetBlocksComputationTime()))
      }
      // Agile-Ant: End
      ...
      if (blockId.isRDD) {
        val taskContext = Option(TaskContext.get()) 
        val taskAttemptId = taskContext.map(_.taskAttemptId()) 
        val endTimeMs = System.currentTimeMillis // The timestamp when the block computation ends
        executorCacheManager.ProfileBlockTaskIdAndComputationTime(blockId, taskAttemptId.get, endTimeMs-startTimeMs)
      }
      // Agile-Ant: End
  ...
  def GetCachedBlocksWithSizes(): scala.collection.mutable.Map[BlockId, Long] = { memoryStore.GetCachedBlocksWithSizes() }
  def GetBlockInfoManager() = blockInfoManager
  def GetMemoryStore() = memoryStore
  def GetTotalSizeOfCachedBlocks() = memoryStore.GetCachedBlocksSize()

  var expectedExecutorStorageCapacity: Long = 0 
  def updateCachingProfiles(cachedRddsNumberOfTargetPartitionsMap : scala.collection.mutable.Map[Int, Int], expectedExecutorStorageCapacity: Long): Unit = {
    this.expectedExecutorStorageCapacity = expectedExecutorStorageCapacity
    memoryStore.UpdateCachingProfiles(cachedRddsNumberOfTargetPartitionsMap, expectedExecutorStorageCapacity)
  }

  var blocksInMigration: scala.collection.mutable.Set[BlockId] = scala.collection.mutable.Set()
  def migrateABlockToExecutor(blockId: BlockId, peer: BlockManagerId): Boolean ={  
    var isSucceed: Boolean = true
    try {
      blocksInMigration += blockId // With this, the block cannot be removed.
      var data: Option[BlockData] = null
      data = getLocalBytes(blockId)
      blockInfoManager.unlock(blockId)
      val tLevel = StorageLevel(
        useDisk = false,
        useMemory = true,
        ...
        deserialized = false,
        replication = 1)
      val buffer = new BlockManagerManagedBuffer(blockInfoManager, blockId, data.get, false, unlockOnDeallocate = false)
      blockTransferService.uploadBlockSync(
        peer.host,
        peer.port,
        peer.executorId,
  ...
  def removeBlock(blockId: BlockId, tellMaster: Boolean = true): Unit = {
    ...
    blockInfoManager.lockForWriting(blockId) match {
      ...
      case Some(info) =>
        if(!blocksInMigration.contains(blockId)){ // Agile-Ant: we add this condition to avoid removing a block while it is being migrated. After migration, it will be removed anyway
