...
private[spark] class ExecutorMonitor(
  ...
  // Agile-Ant: Start
  def GetExecutorsByIds(executorIds: Set[Int]): Seq[(String, Int)] = { //SparkWithControl
    val execs = executors.asScala.filter{ case (name, exec) => name != "driver" && executorIds.contains(exec.resourceProfileId) }.map{ case (name, exec) => (name, exec.resourceProfileId)}.toSeq 
    execs
  }
  // Agile-Ant: End
