...
abstract class RDD[T: ClassTag](
  ...
  // Agile-Ant: Start
  def AutocacherCache(): this.type = { 
    if (storageLevel == StorageLevel.NONE) {
      sc.cleaner.foreach(_.registerRDDForCleanup(this))
      sc.persistRDD(this)
    }
    storageLevel = StorageLevel.MEMORY_ONLY // only cache blocks in memory without serialization
    this
  }
  // Agile-Ant: End

  private def persist(newLevel: StorageLevel, allowOverride: Boolean): this.type = { 
    // Agile-Ant: disable the default cache instrumentation. We disable the default caching decisions (i.e. cache and unpersist datasets) of the application developers
    /*
   ...
   */
   ...
   // Agile-Ant: Start
   def AutocacherUnpersist(): this.type = { // unpersist is done by the Auto-cacher
    logInfo(s"Removing RDD $id from persistence list")
    sc.unpersistRDD(id, false)
  ...
  def unpersist(blocking: Boolean = false): this.type = { 
    // Agile-Ant: disable the default unpersist instrumentation. We disable the default caching decisions (i.e. cache and unpersist datasets) of the application developers
    /*
    logInfo(s"Removing RDD $id from persistence list")
