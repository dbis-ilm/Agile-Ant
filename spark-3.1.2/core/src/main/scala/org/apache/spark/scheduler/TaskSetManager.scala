...
import org.apache.spark.agile_ant.driver_side.ClusterCacheManager // Agile-Ant
...
private[spark] class TaskSetManager(
  ...
  def handleSuccessfulTask(tid: Long, result: DirectTaskResult[_]): Unit = {
    ...
    ClusterCacheManager.SetTaskInfo(tid, info.duration) // Agile-Ant: send the execution time of the task to the Cluster-cache Manager, with which it calculates the BenefitRatio (Eq. 2 in Sect. 6.1.1 of the paper)
