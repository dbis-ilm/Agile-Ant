...
private[spark] class DAGScheduler(
  ...
  private def createResultStage(
    ...
    AgileAntHandleJobStart(jobId, rdd) // Agile-Ant: we consider this point as the end of the execution of the previous job, i.e. immediately before executing the current job
    val (shuffleDeps, resourceProfiles) = getShuffleDependenciesAndResourceProfiles(rdd)
    val resourceProfile = mergeResourceProfilesForStage(resourceProfiles)
    checkBarrierStageWithDynamicAllocation(rdd)
    checkBarrierStageWithNumSlots(rdd, resourceProfile)
  ...
  // Agile-Ant: Start
  private def AgileAntHandleJobStart(jobId: Int, rdd: RDD[_]): Unit = { 
    if(jobId == 0){
      AgileAntConf.Set(sc)
      ClusterCacheManager.init(sc)
      ScaleOutManager.init(sc) // initialize the Scale-out Manager before starting the first job (i.e. job_0)
      AutoCacher.init()
    }
    else {
      ClusterCacheManager.CalculateStatisticsAtJobEnd(jobId) // calculate the computation time and size of the datasets that are cached in the last job, i.e. the recently cached datasets
    }
    
    AutoCacher.HandleJobStart(jobId, rdd) // the Auto-cacher: before job execution (Sect. 6.1 of the paper)
    
    ScaleOutManager.AnalyzeAndTakeAction() // the Scale-out Manager: when a dataset is beneficial in the final evaluation after it was previously marked as non-beneficial in the early evaluation, the cluster cache is extended to fit the dataset
  }
  // Agile-Ant: End
