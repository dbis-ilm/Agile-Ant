...
import org.apache.spark.agile_ant.MessageType.MessageType
// Agile-Ant: End
...
class BlockManagerMasterEndpoint(
  ...
  override def receiveAndReply(context: RpcCallContext): PartialFunction[Any, Unit] = {
    ...
    case ReceiveAMessageFromAnExecutorCacheManager(messageType, executorId, messageContent) => 
      ClusterCacheManager.ReceiveAMessageFromAnExecutorCacheManager(messageType, executorId, messageContent)
      context.reply(true)

    case SendAMessageToExecutorCacheManagers(messageType, messageContent, executorIds) => 
      sendAMessageToExecutorCacheManager_s(messageType, messageContent, executorIds)
      context.reply(true)
  ...
  // Agile-Ant: Start
  private def sendAMessageToExecutorCacheManager_s(messageType: MessageType, messageContent: Any, executorIds: Set[Int] = Set()): Unit = { // Agile-Ant: 
    blockManagerInfo.values.map { bm =>
      if(!bm.blockManagerId.isDriver && (executorIds.size == 0 || executorIds.contains(bm.blockManagerId.executorId.toInt))) {
        bm.storageEndpoint.ask[Boolean](ReceiveAMessageFromTheClusterCacheManager(messageType, messageContent))
        }
    }
  }
  // Agile-Ant: End
  ...
  private def register(
    ...
    // Agile-Ant: Start
    if(!id.isDriver){ 
      ScaleOutManager.NewBlockManagerIsReady(id) // notify the Scalue-out Manager about the arrival of a new executor
    }
    // Agile-Ant: End
