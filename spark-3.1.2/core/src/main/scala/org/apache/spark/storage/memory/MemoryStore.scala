...
import org.apache.spark.agile_ant.executor_side.ExecutorCacheManager 
// Agile-Ant: End
...
private[spark] class MemoryStore(
  ...
  var executorCacheManager: ExecutorCacheManager = null
  def SetExecutorCacheManager(executorCacheManager: ExecutorCacheManager): Unit = {
    this.executorCacheManager = executorCacheManager
  }

  import scala.collection.JavaConverters._ 
  def GetCachedBlocksWithSizes(): scala.collection.mutable.Map[BlockId, Long] = { 
    var cachedBlocksWithSizesMap: scala.collection.mutable.Map[BlockId, Long] = scala.collection.mutable.Map()  
    entries.synchronized{
      for((blockId, blockEntry) <- entries.asScala){
        if(blockId.isRDD)
          cachedBlocksWithSizesMap += (blockId -> blockEntry.size)
      }
    }
    cachedBlocksWithSizesMap
  }

  def GetCachedBlocks(): scala.collection.mutable.Set[BlockId] = { 
    var cachedBlocksSet: scala.collection.mutable.Set[BlockId] = scala.collection.mutable.Set()  
    entries.synchronized{
      for((blockId, blockEntry) <- entries.asScala){
        if(blockId.isRDD)
  ...
  def GetBlocksToMigrate(migratedBlocksSet : scala.collection.mutable.Set[BlockId]): (scala.collection.mutable.Set[BlockId], Long, Int) =  {
    entries.synchronized{
      var splitIndexsSet : scala.collection.mutable.Set[Int] = scala.collection.mutable.Set()
      for((blockId, blockEntry) <- entries.asScala){
        if(blockId.isRDD){
          splitIndexsSet += blockId.asRDDId.get.splitIndex // splitIndexsSet is the partition number. For example, a block D_5_0 is a block belongs to dataset D_5 and its partition number is 0. We rely on this number to construct the execution lineage. For example, D_5_0 and D_41_0 blocks belong to the same execution lineage because they share the same splitIndexs and both shall be migrated together.
        }
      }

      for(migratedBlockId <- migratedBlocksSet){
        if(migratedBlockId.isRDD){
          splitIndexsSet -= migratedBlockId.asRDDId.get.splitIndex // exclude the splitIndex of the blocks that are under migration but not evicted yet. The block is evicted after it is successfully migrated. See migrateCachedBlocks method in ExecutorCacheManager.scala
        }
      }
      
      for(splitIndex <- splitIndexsSet){
        var foundLockedBlock: Boolean = false
        var selectedBlocksForMigrationSet : scala.collection.mutable.Set[BlockId] = scala.collection.mutable.Set()
        var blocksSize: Long = 0
        for((blockId, blockEntry) <- entries.asScala){
          if(!foundLockedBlock){
            if(blockId.isRDD && blockId.asRDDId.get.splitIndex == splitIndex){
              if(blockInfoManager.isLocked(blockId)){
                foundLockedBlock = true
              }
              else{
                selectedBlocksForMigrationSet += blockId
                blocksSize = blocksSize + blockEntry.size
              }
            }
          }
        }
        ...
        if(!foundLockedBlock){
          return (selectedBlocksForMigrationSet, blocksSize, splitIndex)
        }
      }
    }  
    return (scala.collection.mutable.Set(), 0, -1)
  }

  
  private var cachedRddsNumberOfTargetPartitionsMap : scala.collection.mutable.Map[Int, Int] = scala.collection.mutable.Map() 
  private var expectedExecutorStorageCapacity: Long = 0 
  def UpdateCachingProfiles(cachedRddsNumberOfTargetPartitionsMap : scala.collection.mutable.Map[Int, Int], expectedExecutorStorageCapacity: Long): Unit = {
    this.requiredMemory = 0
    this.cachedRddsNumberOfTargetPartitionsMap = cachedRddsNumberOfTargetPartitionsMap
    this.expectedExecutorStorageCapacity = expectedExecutorStorageCapacity * 1024 * 1024
    this.predictedRddFullTargetSizeMap.clear()
  }

  private var requiredMemory: Long = 0 
  def GetRequiredMemory(): Long = { 
    val reqMem: Long = this.requiredMemory
    this.requiredMemory = 0
    reqMem
  } 
  
  private var predictedRddFullTargetSizeMap: scala.collection.mutable.Map[Int, Long] = scala.collection.mutable.Map()
  def GetPredictedRddFullTargetSizeMap(): scala.collection.mutable.Map[Int, Long] = predictedRddFullTargetSizeMap

  
  val executorCores = conf.get(EXECUTOR_CORES)
  val earlyEvaluationTheshold = this.executorCores * 2
  private def evaluateRddCachedBlocksAndTakeAction(): Unit = { 
    if(this.cachedRddsNumberOfTargetPartitionsMap.size == 0 || this.expectedExecutorStorageCapacity == Long.MaxValue){ return }
    entries.synchronized{
      var predictedRequiredStorageMemory: Long = getRddCachedBlocksNumberAndSize(-1)._2
      for((rddId, targetNumberOfCachedBlocks) <- this.cachedRddsNumberOfTargetPartitionsMap){
        val x: (Int, Long) = getRddCachedBlocksNumberAndSize(rddId)
        val numberOfBlocks = x._1
        val sizeOfBlocks = x._2
        val predictedFullTargetSize: Long = if(numberOfBlocks > earlyEvaluationTheshold ) targetNumberOfCachedBlocks * sizeOfBlocks / numberOfBlocks else 0
        predictedRequiredStorageMemory += predictedFullTargetSize
        this.predictedRddFullTargetSizeMap += (rddId -> predictedFullTargetSize)
      }
      
      if(predictedRequiredStorageMemory > this.expectedExecutorStorageCapacity){
        requiredMemory = predictedRequiredStorageMemory - this.expectedExecutorStorageCapacity
        this.expectedExecutorStorageCapacity = Long.MaxValue
      }
    }
  }

  private def getRddCachedBlocksNumberAndSize(rddId: Int): (Int, Long) = { 
    var numberOfBlocks: Int = 0
    var sizeOfBlocks: Long = 0
    for((blockId, blockEntry) <- entries.asScala){
      if(blockId.isRDD){ 
        if(blockId.asRDDId.get.rddId == rddId || (rddId == -1 && !cachedRddsNumberOfTargetPartitionsMap.contains(blockId.asRDDId.get.rddId)) ){
          numberOfBlocks += 1
          sizeOfBlocks += blockEntry.size
        }
      }
    }
    (numberOfBlocks, sizeOfBlocks)
  }

  def GetCachedBlocksSize(): Long = { 
    var sizeOfBlocks: Long = 0
    for((blockId, blockEntry) <- entries.asScala){
      sizeOfBlocks += blockEntry.size
    }
    scala.math.ceil(sizeOfBlocks.toDouble).toLong
  }

  def GetBlockSize(blockId: BlockId) = if (entries.asScala.contains(blockId)) entries.asScala(blockId).size else 0
  ...
  def putBytes[T: ClassTag](
    ...
    if (memoryManager.acquireStorageMemory(blockId, size, memoryMode)) {
      ...
      evaluateRddCachedBlocksAndTakeAction() // Agile-Ant: after caching (2 * #cores) blocks, we check if the memory is enough to cache the target number of blocks
   ...
   *         memory).
   */
  var s_estimated_size_B : Long = 0 // Agile-Ant: the size of the cached block if it was successfully cached or, otherwise, its predicted size
  private def putIterator[T](
    ...
    s_estimated_size_B = 0 // Agile-Ant: initial value
    ...
    var numberOfRemainingTuples = 0 
    while (values.hasNext) { numberOfRemainingTuples +=  1; values.next();} 
    ...
    if (keepUnrolling) {
      ...
      if (keepUnrolling) {
        ...
        evaluateRddCachedBlocksAndTakeAction() // Agile-Ant
      ...
      // Agile-Ant: Start
      val s_size = valuesHolder.estimatedSize() 
      if(elementsUnrolled == 0)
        s_estimated_size_B = s_size // successfully cached
      else 
        s_estimated_size_B = (s_size + ( s_size * numberOfRemainingTuples / elementsUnrolled)) // failed to be cached. So, we linearly scale the incomplete block size
  ...
  private[spark] def evictBlocksToFreeSpace( 
    ...
    // Agile-Ant: we modify this method to evict blocks starting from the one with the least priority (Sect. 7.1.2 of the paper)
    assert(space > 0)
    memoryManager.synchronized {
      var freedMemory = 0L
      ...
      // Agile-Ant: Start
      var evictingBlockRDDId = -1
      if(!rddToAdd.isEmpty){
        evictingBlockRDDId = blockId.flatMap(getRddId).get 
      }
      // Agile-Ant: End
      
      // Agile-Ant: below, we remove a part of the left hand side of the condition because we allow evicting a block from the same RDD (refer to ExecutorCacheManager.scala)
      /*
      ...
      */
      ...
      val isNonRddEviction: Boolean = blockId == None || evictingBlockRDDId == -1 
      
      val evictionPriorityList: List[Int] = if(isNonRddEviction) List(-1) else this.executorCacheManager.GetEvictionPriorityList(evictingBlockRDDId) // the Executor-cache Manager prioritizes the datasets (i.e. RDDs) and the policy is applied here
      // Agile-Ant: End

      entries.synchronized {
        for (rddId <- evictionPriorityList){ // Agile-Ant: iterate over the priority list, starting from the RDD with the least priority
          val iterator = entries.entrySet().iterator()
          while (freedMemory < space && iterator.hasNext) {
            val pair = iterator.next()
            val blockId = pair.getKey
            val entry = pair.getValue
            if (isNonRddEviction || !blockId.isRDD || blockId.asRDDId.get.rddId == rddId){ // Agile-Ant: the block can be selected for eviction if it does not belong to an RDD (i.e. from execution) or it belongs to the same RDD in the round
      ...
      def dropBlock[T](blockId: BlockId, entry: MemoryEntry[T]): Unit = {
        ...
        // Agile-Ant: Start
        if(blockId.isRDD) { 
          // we profile the evictions which will be sent to the Cluster-cache Manager at the end of the job execution. If evictions occur, the Cluster-cache Manager considers the executor as an overloaded executor
          this.executorCacheManager.ProfileEviction(blockId)
          this.executorCacheManager.ProfileBlockSize(blockId, getSize(blockId))
        }
        // Agile-Ant: End
