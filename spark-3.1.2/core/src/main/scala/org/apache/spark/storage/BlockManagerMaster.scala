...
import org.apache.spark.agile_ant.MessageType.MessageType // Agile-Ant
...
class BlockManagerMaster(
  ...
  def SendAMessageToExecutorCacheManager_s(messageType: MessageType, messageContent: Any, executorIds: Set[Int] = Set()): Unit = { 
    driverEndpoint.ask[Boolean](SendAMessageToExecutorCacheManagers(messageType, messageContent, executorIds))
  }

  def SendAMessageToTheClusterCacheManager(messageType: MessageType, executorId: Int, messageContent: Any): Unit = { 
    driverEndpoint.askSync[Boolean](ReceiveAMessageFromAnExecutorCacheManager(messageType, executorId, messageContent)) 
  }
