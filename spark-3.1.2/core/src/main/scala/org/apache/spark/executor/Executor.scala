...
private[spark] class Executor(
  ...
  class TaskRunner(
    ...
    override def run(): Unit = {
      ...
      // Agile-Ant: Start
      if(numRunningTasks == 0) { // the executor finished executing all the tasks in the job
        if(SparkEnv.get.blockManager.executorId != "driver"){
          SparkEnv.get.blockManager.executorCacheManager.Send_Metrics_List_To_The_Cluster_Cache_Manager() // the Executor-cache Manager sends the metrics list to the Cluster-cache Manager at the end of the job execution (Sect. 7.2 of the paper)
        }
      }
      // Agile-Ant: End
    }
