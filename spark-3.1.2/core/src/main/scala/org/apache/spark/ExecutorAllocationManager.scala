...
private[spark] class ExecutorAllocationManager(
  ...
  // Agile-Ant: Start
  private var targetNumberOfExecutors : Int = -1 // the Scale-out Manager requests for executors by setting this value using the SetNewTargetNumberOfExecutors function
  def SetNewTargetNumberOfExecutors(newTargetNumberOfExecutors: Int): Unit = {
  	targetNumberOfExecutors = newTargetNumberOfExecutors
  }
  // Agile-Ant: End
  ...
  private def addExecutors(maxNumExecutorsNeeded: Int, rpId: Int): Int = {
    ...
    // Agile-Ant: Start
    if(numExecutorsTarget != -1){ 
      // if numExecutorsTarget is not equal to -1, it means that the Scale-out Manager requested additional executors      
      numExecutorsTargetPerResourceProfileId(rpId) = targetNumberOfExecutors // request for additional executors from the cluster manager. E.g., if there are two executors and three more are required, the Scalue-out Manager sets this value to five
      numExecutorsTarget = -1 // avoid requesting for the added executors again
    }
    // Agile-Ant: End
  ...
  def RemoveExecutors(executorIds: Set[Int]): Unit = {
    removeExecutors(executorMonitor.GetExecutorsByIds(executorIds))
  }
  // Agile-Ant: End
